        ...
        <li v-for="r in references.items" :key="r.id" class="refs__cell" :data-ref="r.id">
          ...
          <div class="refcard">
            <span class="refcard__under" aria-hidden="true">
              <span class="refcard__seam emisija">{{ r.urlLabel }}</span>
            </span>
...
.refs__strip-wrap::after {
  ...
  right: 0;
  width: 2.5rem;
  /* fallback literal = --list at alpha 0; the relative-color line below
     tracks the token where supported */
  background: linear-gradient(to right, rgb(245 242 235 / 0), var(--list));
  background: linear-gradient(to right, rgb(from var(--list) r g b / 0), var(--list));
  pointer-events: none;
}
...
@media (min-width: 900px) {
  ...
  .refs__strip-wrap::after {
    ...
    width: auto;
    height: 10rem;
    background: linear-gradient(to bottom, rgb(245 242 235 / 0), var(--list) 72%);
    background: linear-gradient(to bottom, rgb(from var(--list) r g b / 0), var(--list) 72%);
  }
  ...
  /* Empty paper under the last card's text, so the fade has card to dissolve
     rather than page-on-page (a gradient to --list over --list is
