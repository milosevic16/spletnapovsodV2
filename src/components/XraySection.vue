...
import { hero, invisible } from '@/content/home'
...
/**
 * The machine layer's lines come from src/lib/machine-facts.ts — the single
 * source that also feeds the build guard, which asserts BIDIRECTIONALLY that
 * every `data-fact` node here byte-equals the emitted <head> and that every
 * enumerated fact is actually displayed. Drift is impossible by construction.
 */
const machineLines = factLines
    ...
    <div class="container">
      ...
      <div class="xray__instrument" :class="{ 'xray__instrument--live': hydrated }">
        ...
        <div ref="machineLayer" class="xray__layer xray__layer--machine">
          <code
            v-for="line in machineLines"
            :key="line.id"
            class="xray__code emisija"
            :data-fact="line.id"
            >{{ line.text }}</code
          >
...
/* <code> is inline by default — blockified explicitly so swapping <p> for the
   guard-checked <code data-fact> element changed nothing about the layout. */
.xray__code {
  display: block;
  color: var(--papir-dim);
