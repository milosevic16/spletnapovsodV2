...
const props = defineProps<{
  /** The annotation — a real, verifiable fact about what the line spans. */
  annotation: string
...
<template>
  <div ref="host" class="prerez" :class="{ 'prerez--dark': props.onDark }">
    ...
    <div ref="textEl" class="prerez__text">
      <span :class="props.emission ? 'emisija' : 'annot'" class="prerez__annot">{{
        props.annotation
      }}</span>
...
<style scoped>
.prerez {
  --prerez-line: var(--mreza-strong);
  --prerez-mark: var(--rez);
  --prerez-annot: var(--grafit-2);
  --prerez-gloss: var(--grafit-2);
}
.prerez--dark {
  --prerez-line: var(--crta-na-temnem);
  --prerez-mark: var(--rez-na-temnem);
  --prerez-annot: var(--rez-na-temnem);
  --prerez-gloss: var(--papir-dim);
}
